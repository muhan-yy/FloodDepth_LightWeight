...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Detect_dyhead, Segment_dyhead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (Detect, Detect_dyhead ,Segment_dyhead)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            def _forward(x):
                ...
                return self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB, Segment_dyhead)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in {Detect, WorldDetect, Segment, Pose, OBB, ImagePoolingAttn, v10Detect, Detect_dyhead , Segment_dyhead}:
            args.append([ch[x] for x in f])
            if m is (Segment, Segment_dyhead):
...
def guess_model_task(model):
    ...
    def cfg2task(cfg):
        ...
        if "detect" in m:
            return "detect"
        if m in ("segment", "segment_dyhead"):
            return "segment"
        # if m == "segment":
        #     return "segment"
        # if m =="segment_dyhead":
    ...
    if isinstance(model, nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, (Segment, Segment_dyhead)):
                return "segment"
