...
class TaskAlignedAssigner(nn.Module):
    ...
    def iou_calculation(self, gt_bboxes, pd_bboxes):
        ...
        # return bbox_iou(gt_bboxes, pd_bboxes, xywh=False, CIoU=True).squeeze(-1).clamp_(0) # 原始 iou
        # return bbox_iou(gt_bboxes, pd_bboxes, xywh=False).squeeze(-1).clamp_(0)   # WIoU v3
        # return shape_iou(gt_bboxes, pd_bboxes).squeeze(-1).clamp_(0) # shape iou
        return (bbox_iou(gt_bboxes, pd_bboxes, xywh=False).squeeze(-1).clamp_(0) + shape_iou(gt_bboxes, pd_bboxes).squeeze(-1).clamp_(0))/2 # WIoU 与 shape iou 取平均
